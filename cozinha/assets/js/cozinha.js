// JavaScript da Cozinha - Sistema de Pedidos
document.addEventListener('DOMContentLoaded', () => {
    const colunas = {
        ...
        'cozinha-finalizado': document.getElementById('coluna-cozinha-finalizado')
    };
    ...
    const contadores = {
        ...
        'cozinha-finalizado': document.getElementById('count-cozinha-finalizado')
    };
    ...
    // Função para determinar se o pedido é da cozinha
    function ehPedidoCozinha(pedido) {
    ...
    async function buscarPedidos() {
        try {
            const response = await fetch('../api/buscar_pedidos_setor.php?setor=cozinha');
    ...
    function exibirPedidos(pedidos) {
        ...
        // Filtrar apenas pedidos da cozinha
        const pedidosCozinha = pedidos.filter(ehPedidoCozinha);

        pedidosCozinha.forEach(pedido => {
            contagem[pedido.status]++;
            
            const colunaId = `cozinha-${pedido.status.toLowerCase().replace(' ', '-')}`;
        ...
        contadores['cozinha-pendente'].textContent = contagem['Pendente'];
        contadores['cozinha-preparo'].textContent = contagem['Em Preparo'];
        contadores['cozinha-pronto'].textContent = contagem['Pronto'];
        contadores['cozinha-finalizado'].textContent = contagem['Finalizado'];
    ...
    function criarCardPedido(pedido) {
        ...
        const itensHtml = pedido.itens
            .filter(item => !ehBebida(item.nome)) // Mostrar apenas itens de comida
            .map(item => `
        ...
        if (pedido.status === 'Pendente') {
            ...
            botaoAcao = `<button class="btn btn-sm btn-pronto-cozinha w-100" onclick="atualizarStatus(${pedido.id}, 'Pronto')">
    ...
    function criarEstadoVazio(colunaId) {
        ...
        switch(status) {
            case 'pendente':
                ...
                break;
            case 'preparo':
                mensagem = 'Nenhum pedido em preparo';
                icone = 'bi-fire';
                break;
            case 'pronto':
                ...
                break;
            case 'finalizado':
                mensagem = 'Nenhum pedido finalizado';
                icone = 'bi-check-all';
                break;
        }
